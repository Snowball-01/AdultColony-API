...
export async function scrapeContent(url: string) {
  try {
    ...
    const xv = new XvideosGet();
    const data: IVideoData = {
      success: true,
      data: {
        title: adultcolony.removeHtmlTagWithoutSpace(xv.title),
        id: xv.id,
        image: xv.image,
        duration: adultcolony.secondToMinute(Number(xv.duration)),
        views: adultcolony.removeHtmlTag(xv.views),
        rating: xv.rating,
        uploaded: xv.publish,
        upvoted: xv.upVote,
      ...
      source: xv.link,
      assets: adultcolony.removeAllSingleQuoteOnArray([xv.embed, xv.thumbnail, xv.bigimg, xv.video])
    };
    return data;

...
export async function scrapeContent(url: string) {
  try {
    ...
    const title = $("h2.g_title")
      .map((i, abc) => {
        return $(abc).text();
      })
      .get();

    const link = $("h2.g_title")
      .map((i, abc) => {
        //return number only
    ...
    const imgSrc = $("img")
      .map((i, el) => $(el).attr("data-cfsrc"))
      .get();
    const imgSrcClean = imgSrc.slice(0, imgSrc.length - 1);

    const imagesSrc = $("div.inner_thumb")
    ...
    for (const abc of title) {
      const objectData: IHentaiFoxSearch = {
        ...
        image: imagesSrc[title.indexOf(abc)],

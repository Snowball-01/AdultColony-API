  ...
  try {
    ...
    class XnxxGet {
      ...
      constructor() {
        ...
        this.thumbnail =
          thumb.match(/html5player.setThumbSlideBig\((.*?)\)/)?.[1] || "None";
        this.bigimg =
          thumb.match(/html5player.setThumbUrl169\((.*?)\)/)?.[1] || "None";
        this.video =
          thumb.match(/html5player.setVideoUrlHigh\((.*?)\)/)?.[1] || "None";
        ...
        this.duration =
          $("meta[property='og:duration']").attr("content") || "None";
        this.views = $("span.metadata").text() || "None";
        this.views = this.views.split("-")[2] || "None";
        this.uploaded =
          $("script[type='application/ld+json']").text() || "None";
        this.uploaded =
          this.uploaded
            .split("uploadDate")[1]
            .split("}")[0]
    ...
    const data: IVideoData = {
      success: true,
      data: {
        title: adultcolony.removeHtmlTagWithoutSpace(x.title),
        id: x.id,
        image: x.image,
        duration: adultcolony.secondToMinute(Number(x.duration)),
        views: adultcolony.removeHtmlTag(x.views),
        rating: x.favVote,
      ...
      assets: adultcolony.removeAllSingleQuoteOnArray([
        x.embed,
        x.thumbnail,
        x.bigimg,
        x.video,
